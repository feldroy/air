...
app = air.Air(cache=RedisCache(url="redis://username:password@localhost.com:6379/0"))
...
async def aabout_page() -> air.Children | air.Html:
    # simple render counter stored in module globals to verify caching
    count: int = globals().get("_redis_async_about_page_renders", 0) + 1
    globals()["_redis_async_about_page_renders"] = count
    ts = time()
